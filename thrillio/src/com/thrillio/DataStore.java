...
public class DataStore {
	...
	public static final int BOOKMARK_TYPES_COUNTS = 3;
	...
	private static Bookmark[][] bookmarks = new Bookmark[BOOKMARK_TYPES_COUNTS][BOOKMARK_COUNT_PER_TYPE];
	...
	private static void loadUsers() {
		users[0] = UserManager.getInstance().createUser(1000, "John", "M", Gender.MALE, "user0@semanticsquare.com",
				"tests", UserType.USER);

		users[1] = UserManager.getInstance().createUser(1001, "Sam", "M", Gender.MALE, "user1@semanticsquare.com",
				"tests", UserType.USER);

		users[2] = UserManager.getInstance().createUser(1002, "Anita", "M", Gender.FEMALE, "user2@semanticsquare.com",
				"tests", UserType.EDITOR);

		users[3] = UserManager.getInstance().createUser(1003, "Sara", "M", Gender.FEMALE, "user3@semanticsquare.com",
				"tests", UserType.EDITOR);

		users[4] = UserManager.getInstance().createUser(1004, "Dheeru", "M", Gender.MALE, "user4@semanticsquare.com",
				"tests", UserType.CHIEF_EDITOR);
	}

	private static void loadWebLinks() {
		bookmarks[0][0] = BookmarkManager.getInstance().createWebLinkManager(2000, "Taming Tiger, Part 2",
				"http://www.javaworld.com/article/2072759/core-java/taming-tiger--part-2.html",
				"http://www.javaworld.com");

		bookmarks[0][1] = BookmarkManager.getInstance().createWebLinkManager(2001,
		...
		bookmarks[0][2] = BookmarkManager.getInstance().createWebLinkManager(2002, "Interface vs Abstract Class",
				"http://mindprod.com/jgloss/interfacevsabstract.html", "http://mindprod.com");

		bookmarks[0][3] = BookmarkManager.getInstance().createWebLinkManager(2003, "NIO tutorial by Greg Travis",
				"http://cs.brown.edu/courses/cs161/papers/j-nio-ltr.pdf", "http://cs.brown.edu");

		bookmarks[0][4] = BookmarkManager.getInstance().createWebLinkManager(2004, "Virtual Hosting and Tomcat",
	...
	private static void loadBooks() {
		bookmarks[2][0] = BookmarkManager.getInstance().createBook(4000, "Walden", 1854,
				new String[] { "Henry David Thoreau" }, "Wilder Publications", BookGenre.PHILOSOPHY, 4.3);

		bookmarks[2][1] = BookmarkManager.getInstance().createBook(4001, "Self-Reliance and Other Essays", 1993,
				new String[] { "Ralph Waldo Emerson" }, "Dover Publications", BookGenre.PHILOSOPHY, 5.0);

		bookmarks[2][2] = BookmarkManager.getInstance().createBook(4002, "Light From Many Lamps", 1988,
				new String[] { "Eichler Watson" }, "Touchstone Lillian", BookGenre.PHILOSOPHY, 5.0);

		bookmarks[2][3] = BookmarkManager.getInstance().createBook(4003, "Head First Design Patterns", 2004,
				new String[] { "Eric Freeman", "Bert Bates", "Kathy Sierra", "Elisabeth Robson" }, "O'Reilly Media",
		...
		bookmarks[2][4] = BookmarkManager.getInstance().createBook(4004, "Effective Java Programming Language Guide",
				2007, new String[] { "Joshua Bloch" }, "Prentice Hall", BookGenre.TECHNICAL, 4.9);
	}
